		...
		if (node.mesh >= 0) {
			...
			for (const auto &primitive : mesh.primitives) {
				...
				if (primitive.attributes.count("TEXCOORD_0")) {
					const auto &uv_accessor = gltf_scene.accessors[primitive.attributes.at("TEXCOORD_0")];
					const auto &uv_view = gltf_scene.bufferViews[uv_accessor.bufferView];
					const auto &uv_buffer = gltf_scene.buffers[uv_view.buffer];
				...
				if (primitive.attributes.count("NORMAL")) {
					const auto &norm_accessor = gltf_scene.accessors[primitive.attributes.at("NORMAL")];
					const auto &norm_view = gltf_scene.bufferViews[norm_accessor.bufferView];
					const auto &norm_buffer = gltf_scene.buffers[norm_view.buffer];
				...
				if (primitive.attributes.count("TANGENT")) {
					const auto &tan_accessor = gltf_scene.accessors[primitive.attributes.at("TANGENT")];
					const auto &tan_view = gltf_scene.bufferViews[tan_accessor.bufferView];
					const auto &tan_buffer = gltf_scene.buffers[tan_view.buffer];
				...
				for (size_t i = 0; i < pos_accessor.count; ++i) {
					...
					// transform position
					glm::vec4 pos = world_transform * glm::vec4(positions[i*3], positions[i*3+1], positions[i*3+2], 1.0f);
					v.pos[0] = pos.x; v.pos[1] = pos.y; v.pos[2] = pos.z;
					
					// UVs
					if (uvs) {
						v.uv[0] = uvs[i*2]; v.uv[1] = uvs[i*2+1];
					}
					
					// transform normal
					if (normals) {
						glm::vec3 norm = normal_matrix * glm::vec3(normals[i*3], normals[i*3+1], normals[i*3+2]);
						norm = glm::normalize(norm);
						v.norm[0] = norm.x; v.norm[1] = norm.y; v.norm[2] = norm.z;
					}
					
					// transform tangent
					if (tangents) {
						glm::vec3 tan = normal_matrix * glm::vec3(tangents[i*4], tangents[i*4+1], tangents[i*4+2]);
				...
				if (primitive.material >= 0) {
					const auto &mat = gltf_scene.materials[primitive.material];
					if (mat.pbrMetallicRoughness.baseColorTexture.index >= 0) {
						const auto &tex = gltf_scene.textures[mat.pbrMetallicRoughness.baseColorTexture.index];
						const auto &img = gltf_scene.images[tex.source];
						GLenum format = (img.component == 4) ? GL_RGBA : GL_RGB;
						p.mat.albedo_tex.emplace();
						p.mat.albedo_tex.value().load(img.image.data(), img.width, img.height, format);
					}
				}
			}
		}
